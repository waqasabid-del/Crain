...
export function createStubClient(overrides: Partial<CairnClient> = {}): CairnClient {
  ...
  return {
    ...
    createTask: vi.fn(unexpected("createTask")),
    listTasks: vi.fn(() => Promise.resolve({ tasks: [] })),
    getTask: vi.fn(unexpected("getTask")),
    updateTask: vi.fn(unexpected("updateTask")),
    setTaskState: vi.fn(unexpected("setTaskState")),
    archiveTask: vi.fn(unexpected("archiveTask")),
    restoreTask: vi.fn(unexpected("restoreTask")),
    listMyTasks: vi.fn(() =>
      Promise.resolve({ todo: [], inProgress: [], inReview: [], blocked: [], done: [] }),
    ),
